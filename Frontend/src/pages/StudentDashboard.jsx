...
const StudentDashboard = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Make an API call to log out the student
      await axios.post('http://localhost:8000/api/student/logout', { refreshToken });

      // Clear tokens and logout using AuthContext
    ...
    const fetchOpportunities = async () => {
      try {
        const [onCampusResponse, offCampusResponse] = await Promise.all([
          axios.get('http://localhost:8000/api/student/oncampus-opportunities'),
          axios.get('http://localhost:8000/api/student/offcampus-opportunities'),
        ]);
  ...
  return (
    <div className="flex h-screen bg-gray-50 position-relative">
      ...
      {/* Mobile Sidebar */}
      <div
        className={`md:hidden fixed top-0 right-0 bottom-0 w-4/5 h-1/2 rounded-md bg-white shadow-md transition-all duration-300 z-40 transform ${
          isSidebarOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div className="flex flex-col h-full">
          <h2 className="text-center py-6 text-xl font-bold border-b">Welcome Student</h2>
            ...
            {onCampusOpportunities.length > 0 ? (
              onCampusOpportunities.map((opportunity, index) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 mb-1">{opportunity.jobProfile}</p>
                  <p className="text-sm text-green-500 font-medium">{opportunity.ctc}</p>
                  <a
